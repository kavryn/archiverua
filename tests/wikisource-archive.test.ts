...
describe("getArchivePageTitle", () => {
  it("returns root archive page for ЦДІАК", () => {
    expect(getArchivePageTitle("ЦДІАК", "Р-34")).toBe("Архів:ЦДІАК");
  });

  it("returns root archive page for ЦДІАЛ", () => {
    expect(getArchivePageTitle("ЦДІАЛ", "34")).toBe("Архів:ЦДІАЛ");
  });

  it("keeps prefix-based title for other archives", () => {
    expect(getArchivePageTitle("ДАХмО", "Р-34")).toBe("Архів:ДАХмО/Р");
  });
});
