...
describe("buildSpravaRow", () => {
  it("builds a row with 5 columns (default)", () => {
    const row = buildSpravaRow(127, "Назва справи", "1920-1930", 5);
    expect(row).toBe("|-\n|[[/127/]]||Назва справи||1920-1930||||");
  });

  it("builds a row with 3 columns", () => {
    const row = buildSpravaRow(5, "Назва", "2000", 3);
    expect(row).toBe("|-\n|[[/5/]]||Назва||2000");
  });

  it("handles empty name and dates", () => {
    const row = buildSpravaRow(1, "", "", 5);
    expect(row).toBe("|-\n|[[/1/]]||||||||");
  });
});
...
describe("buildNewOpysPage", () => {
  ...
  it("creates page with single data row when sprava=1", () => {
    ...
    // Should NOT have a placeholder row
  ...
  it("creates page with placeholder row + actual row when sprava!=1", () => {
    ...
    // Placeholder row should come before actual row
...
[[Категорія:Тест]]`;
  ...
  it("parses page with one row", () => {
    ...
    expect(parsed.rows[0].spravaNum).toBe(1);
  ...
  it("parses page with multiple rows", () => {
    ...
    expect(parsed.rows.map((r) => r.spravaNum)).toEqual([1, 3, 5]);
  });
...
describe("insertSpravaRow", () => {
  const makeParsed = (rows: { raw: string; spravaNum: number | null }[]) => ({
    before: "before\n",
  ...
  it("returns unchanged when sprava already exists", () => {
    ...
    const original = insertSpravaRow(parsed, 999, "X", "X"); // get baseline format
    const parsed2 = makeParsed([makeRow(1), makeRow(5)]);
    const result = insertSpravaRow(parsed2, 5, "Different", "Different");
  ...
  it("matches column count from header", () => {
    const parsed = {
      ...makeParsed([]),
      headerLine: "!A!!B!!C",
      columnCount: 3,
    };
    const result = insertSpravaRow(parsed, 1, "Name", "2020");
    // Row should have exactly 3 cells
    const rowMatch = result.match(/\|\[\[\/1\/\]\]\|\|Name\|\|2020/);
    expect(rowMatch).not.toBeNull();
    // Should NOT have more || separators
    const rowLine = result.split("\n").find((l) => l.includes("[[/1/]]"))!;
    const separators = (rowLine.match(/\|\|/g) || []).length;
    expect(separators).toBe(2); // 3 columns = 2 separators
  });
});
