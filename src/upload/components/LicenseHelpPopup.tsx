      ...
      <Dialog open={open} onClose={() => setOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white shadow-xl">
            ...
            <div className="flex flex-col divide-y divide-zinc-300 px-6">
              {ALL_OPTIONS.map((opt) => (
                <div key={opt.value} className="py-4">
                  ...
                  {opt.note && (
                    <p className="mt-1 text-sm text-zinc-700"><span className="font-bold
                    not-italic">Використання:</span> {opt.note}</p>
                  )}
                  <p className="mt-1 text-sm text-zinc-700"><span className="font-bold not-italic">Офіційний
                  опис:</span> {opt.helpText}</p>
                </div>
              ))}
            </div>
          </DialogPanel>
